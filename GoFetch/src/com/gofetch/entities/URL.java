...
public class URL implements Serializable {
	...
	private Integer data_entered_by; // 0 = sourced from SeoMoz, 1 = user
										// entered, 2 = ?? majestic ??
	private boolean seomoz_url;

	private Integer users_user_id; // points to a client's id, so that this url
									// can be assigned to a client
	private Integer client_category_id; // points to a client-specific category,
										// so this url can be assigned to a
										// client's category
	// private Integer client_category_users_user_id; // allows a client's
	...
	// delete when related url is deleted,
	...
	/*
	 * decreases social_data_freq by 1. if its already at max - then remains at
	 * max
	 */
	public void decreaseSocialCrawlFrequency() {

		if (GoFetchConstants.MONTHLY_FREQ == social_data_freq) // if at max,
																// just return
			return;
	...
	/*
	 * increases social_data_freq by 1 if its already checking at daily,
	 */
	public void increaseSocialCrawlFrequency() {

		if (GoFetchConstants.DAILY_FREQ == social_data_freq) // if already
																// checking
																// daily - just
																// return
			return;
