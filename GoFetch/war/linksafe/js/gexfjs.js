...
function displayNode(_nodeIndex, _recentre) {
    ...
    if (_nodeIndex != -1) {
        ...
        ////////////////////////
        // AD - 1-3-12 - I've really hacked around in here to make the L column display various bits
        
        var _strImage = ''; //temp fix to rotate a few different shrunk images in L column.
       
        iTempImgCounter += 1;
        
        if(iTempImgCounter > 3){ // roll back to first image
            iTempImgCounter = 1;
        }
            
        
        switch(iTempImgCounter)
        {
            case 1:
                _strImage = '\"./img/TheBigProjectShrink.jpg\"';
                break;
                
            case 2:
                _strImage = '\"./img/PimpYourFinancesShrink.jpg\"';
                break;
                
            default:
                _strImage = '\"./img/VirginMoneyShrink.jpg\"';
        }
        
        //var strImageLoad = '\"./img/loading.gif\"'; // use later for any display we need as loading...
        
        
        _str += '<a href= \"' + _d.id + '\" > <img id= \'imgShrunkWeb\' src = ' + _strImage + 'alt= \" \" width=\"250\" height=\"180\"/> </a>';         
        _str += '<h3><div class="largepill" style="background: ' + _d.color.base +'"></div>' + _d.label + '</h3>';
        
        _str += '<ul><br><br><br><br><li><b>id</b> : ' + '<a href=' + _d.id  + ' >' + _d.id + '</a>' + ' </li>';
...
function traceMap() {
    ...
    for (var i in GexfJS.graph.nodeList) {
        ...
        if (_d.visible && _d.withinFrame) {
            if (i != _centralNode) {
                ...
                if (_fs > _limTxt) {
                    GexfJS.ctxGraphe.fillStyle = ( ( i != GexfJS.params.activeNode ) && _tagsMisEnValeur.length && ( ( !_d.isTag ) || ( _centralNode != -1 ) ) ? "rgba(255,255,255,0.1)" : "rgb(255,255,255)" );
    ...
    if (_centralNode != -1) {
        ...
        var _fs = Math.max(_limTxt + 2, _dnc.coords.real.r * _textSizeFactor) + 18;
        GexfJS.ctxGraphe.font = "bold " + Math.floor( _fs )+"px Arial";
        GexfJS.ctxGraphe.textAlign = "center";
        GexfJS.ctxGraphe.textBaseline = "middle";
        GexfJS.ctxGraphe.fillStyle = "rgba(0,0,0,0.9)";
    ...
    GexfJS.ctxMini.strokeStyle = "rgb(0,0,0)";
    GexfJS.ctxMini.lineWidth = 10;
    GexfJS.ctxMini.fillStyle = "rgba(0,0,0,0.9)";
    GexfJS.ctxMini.beginPath();
